...
Compositionality; only ratios meaningful:
How to make ratios meaningful subtraction operations?
Working in log-scale --> that is why we take log.

The current scale is arbitrary and caused by the effect of library sizes:
How to center the data distribution?
Subtracting mean, because we are working in log-scale we use geometric mean.
