...
- This is the final session. These are the topics that we have covered.
- Now you should have idea on microbiome data science ecosystems, the overview of microbiome data science workflows and how Bioconductor positions in it.
- Today we go through alpha and beta diversity and differential abundance analysis. With these, you can already do simple analysis workflow.

:::

## tidyomics [@Hutchison2024]
...
::: {.notes}
- There were great questions last time. I want to come back to this tidymodels question.
- There is this project in Bioconductor, that develops methods in tidy syntax for SummarizedExperiment objects
- As we are using TreeSE, we can utilize the methods.
- This means that we can operate and create these tidy pipelines.
- This example is very simple as is only intended to show the possibilities
:::
...
::: {.notes}
As a short recap, this is the overview of the workflow.
:::
...
::: {.notes}
- Then we do data wrangling such as apply agglomeration and transformations. So basically, we are now in the statistical analysis part.
:::
...
::: {.notes}
- Alpha diversity refers to diversity within a sample.
- All metrics measure evenness and richness, but they whey them differently
- Evenness: How evenly the abundances are distributed?
- Richness: How many different taxa are present?
- Here are three different samples... (go them through)
- After calculating alpha diversity, we can then compare alpha diversities between samples.
:::
...
::: {.notes}
- Beta diversity refers to diversity within a samples.
- Usual way is to apply these ordination methods that projects high-dimensional data into low dimension while preserving information.
- The goal is to find patterns from the sample grouping.
:::
...
df <- data.frame(
  group = rep(c("Control", "Treated"), each = 20),
  abundance = c(
    rnorm(20, mean = 50, sd = 10),   # Control group
    rnorm(20, mean = 80, sd = 15)    # Treated group (higher abundance)
  )
)
...
::: {.notes}
- In differential abundance analysis, we go one by one each taxa and ask question: "does the abundance of this taxon differs between the groups?"
- This related to the question from the last session about the transformation. Microbiome data is compositional which is why commonly centered log-ratio transformation is applied. However, there is more evidence that the simplest approaches such as relative transformation and linear model work better in DAA, and CLR can be harmful.
- In ML applications simple presence/absence works the best, but for instance, in correlation CLR can be useful as it makes the data behave better (more nrmally distributed and not constrained to 0).
:::
...
tse <- peerj13075
tse <- addAlpha(tse)
plotBoxplot(tse, col.var = "shannon_diversity", x = "Diet")
```
...
- [OMA online book](https://bioconductor.org/books/release/OMA/){preview-link="true"}
- [Slides](https://microbiome.github.io/outreach/)
- "miaverse" channel on Bioconductor Zulip


::: notes
- Online book and slides are freely available
- You can find us in Zulip. Also email or GitHub. As I mentioned this is community effort so even discussing in Zulip will help us to know possible errors and help others to learn from your experience. Also you will get guidance.
:::

## European Bioconductor Conference 2026 will have live streaming!

```{r}
#| label: show_eurobioc2
#| fig-width: 10
#| fig-height: 10
#| fig-align: right
#| eval: true
