...
pkgs <- c("BiocPkgTools", "ggrepel", "tidyverse")
...
se_pkgs <- se_pkgs[order(se_pkgs[["N"]], decreasing = TRUE), , drop = FALSE]
se_pkgs[["Field"]] <- factor(se_pkgs[["Field"]])
ggplot(se_pkgs, aes(x = N, y = fct_inorder(Field), fill = fct_inorder(Field))) +
    geom_col(width = 0.7, colour = "black") +
    scale_fill_brewer(palette = "Paired", name = "Field") +
    geom_text(aes(label = N), hjust = -0.5, size = 4.5) +  # labels at bar ends
    labs(x = "Number of packages", y = NULL) +
    coord_cartesian(clip = "off") +            # allow labels past bar end
